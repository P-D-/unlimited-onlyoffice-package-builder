...
- Find all the `P-D-` strings and replace them to your Github organisation/user. Example: `acmeoo`.
- Find all the `vcrp` strings and replace them to your branding (no spaces or fancy characters). Example: `acme`. This will be used for both Git tags and Debian package suffix.
...
- Given a **x.y.z.t** version that you want to build: ( Example: `8.1.2.3` )
  - Find all the `8.3.3` strings and replace them to **x.y.z**. Example: `8.1.2`
  - Find all the `18` strings and replace them to **t**. Example: `3`
...
```
cd ~/onlyoffice_repos
git clone git@github.com:P-D-/unlimited-onlyoffice-package-builder.git
```
...
```
cd ~/onlyoffice_repos
git clone git@github.com:P-D-/build_tools.git
```
...
```
cd ~/onlyoffice_repos
git clone git@github.com:P-D-/server.git
```
...
```
cd ~/onlyoffice_repos
git clone git@github.com:P-D-/web-apps.git
```
...
```
The most recent 8.3.3 version is:

8.3.3-18

so that's the version that we will be using.
We just replace the hyphen with a dot. 8.3.3-18 is now: 8.3.3.18.
...
```
cd ~/onlyoffice_repos/build_tools
git checkout tags/v8.3.3.18 -b 8.3.3.18-vcrp
```
...
```
sed -i 's/unlimited_organization = "btactic-oo"/unlimited_organization = "P-D-"/g' scripts/base.py
sed -i 's/unlimited_tag_suffix = "-btactic"/unlimited_tag_suffix = "-vcrp"/g' scripts/base.py
```
.
...
```
git push origin 8.3.3.18-vcrp
git tag -a 'v8.3.3.18-vcrp' -m '8.3.3.18-vcrp'
git push origin v8.3.3.18-vcrp
```
...
mkdir ~/build-oo
cd ~/build-oo
git clone https://github.com/P-D-/unlimited-onlyoffice-package-builder
cd unlimited-onlyoffice-package-builder
git checkout v0.0.1
# Ignore detached HEAD message
./onlyoffice-package-builder.sh --product-version=8.3.3 --build-number=18 --unlimited-organization=P-D- --tag-suffix=-vcrp --debian-package-suffix=-vcrp
```

### Final deb package

The final `onlyoffice-documentserver_8.3.3-18-vcrp_amd64.deb` deb package can be found at: `~/build-oo/unlimited-onlyoffice-package-builder/document-server-package/deb/` directory.
...
Visit [https://github.com/P-D-/unlimited-onlyoffice-package-builder/actions](https://github.com/P-D-/unlimited-onlyoffice-package-builder/actions) and click on the **I understand my workflows, go ahead and enable them** button.
...
git checkout main
sed -i 's/DEBIAN_PACKAGE_SUFFIX: -btactic/DEBIAN_PACKAGE_SUFFIX: -vcrp/g' .github/workflows/build-release-debian-11.yml
sed -i 's/TAG_SUFFIX: -btactic/TAG_SUFFIX: -vcrp/g' .github/workflows/build-release-debian-11.yml
git add .github/workflows/build-release-debian-11.yml
git commit -m 'Use vcrp as a suffix in Github Actions'
...
git tag -a 'builds-debian-11/8.3.3.18' -m 'builds-debian-11/8.3.3.18'
git push origin 'builds-debian-11/8.3.3.18'
```
.

Release based on Github Actions which you can check in: [https://github.com/P-D-/unlimited-onlyoffice-package-builder/actions](https://github.com/P-D-/unlimited-onlyoffice-package-builder/actions) should end succesfully after about 2h30m build time.

Check the new release at: [https://github.com/P-D-/unlimited-onlyoffice-package-builder/releases](https://github.com/P-D-/unlimited-onlyoffice-package-builder/releases).
